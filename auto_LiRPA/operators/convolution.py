...
class BoundConv(Bound):
    ...
    def interval_propagate(self, *v, C=None):
        ...
        # Propagating L2 norm perturbation for intermediate layers.
        if self is not None and not self.is_final_node and self.output_rho is None:
            with torch.no_grad():
                if norm == 2.0:
                    self.input_rho = torch.tensor([eps]*center.shape[0]).to(center.device)
                else:
                    for input_node in self.inputs:
                        if hasattr(input_node, 'output_rho') and input_node.output_rho is not None:
                            self.input_rho = input_node.output_rho
                            break
                if self.input_rho is not None:
                    self.output_rho = self.input_rho * power_iteration(weight=weight, input_shape=h_U.shape, 
                                                                       output_shape=upper.shape, stride=self.stride, 
