...
class BoundTwoPieceLinear(BoundOptimizableActivation):
    def __init__(self, attr=None, inputs=None, output_index=0, options=None):
        ...
        # Store the input center of this layer.
        self.xc = None
    ...
    def init_opt_parameters(self, start_nodes):
        ...
        # Initialize optimizable variables for SDP-CROWN.
        self.lam = OrderedDict()
        ...
        for start_node in start_nodes:
            ...
            if isinstance(output_shape, (list, tuple)):
                if len(output_shape) > 1:
                    ...
                    unstable_conv = list(output_shape)
                else:
                    size_s = output_shape[0]
            else:
                size_s = output_shape
            
            # Initialize the SDP-CROWN variables.
            lam_shape = None
            # If the output node is conv, check whether init value is correct.
            if ns in self.patch_size:
                lam_shape = list(self.patch_size[ns])
                if unstable_conv is not None:
                    lam_shape = [unstable_conv[0], batch_size, *unstable_conv[1:], *lam_shape[len(output_shape)+1:]]
                else:
                    lam_shape[0] = size_s
                lam_shape = tuple(lam_shape)
            ...
            if sparsity <= minimum_sparsity * size_s and self.use_sparse_spec_alpha:
                ...
                # lam shape is (2, sparse_spec, batch)
                
                if lam_shape:
                    self.lam[ns] = torch.ones([self.alpha_size, *lam_shape[:4]],
                                    dtype=torch.float, device=ref.device, requires_grad=True)
                else:
                    self.lam[ns] = torch.ones([self.alpha_size, sparsity + 1, batch_size],
                                    dtype=torch.float, device=ref.device, requires_grad=True)
                ...
                # lam shape is (2, spec, batch).
                
                if lam_shape is not None:
                    self.lam[ns] = torch.ones([self.alpha_size, *lam_shape[:4]],
                                    dtype=torch.float, device=ref.device, requires_grad=True)
                else:
                    self.lam[ns] = torch.ones([self.alpha_size, size_s, batch_size],
                                    dtype=torch.float, device=ref.device, requires_grad=True)
    ...
    def select_lam_by_idx(self, last_lA, last_uA, unstable_idx, start_node):
        # The out_shape might be sparse (sparse-spec alpha), where the non-zero values are indexed by self.alpha_lookup_idx.
        if unstable_idx is not None:
            # print(f'relu layer {self.name}, start_node {start_node}, unstable_idx {type(unstable_idx)} alpha idx {self.alpha_lookup_idx[start_node.name].size()}')
            if self.alpha_lookup_idx is not None:
            ...
            if isinstance(unstable_idx, tuple):
                # Start node is a conv node.
                # selected_lam = target[start_node.name]
                selected_lam = self.lam[start_node.name]
                if isinstance(last_lA, Tensor) or isinstance(last_uA, Tensor):
                    # Start node is a conv node but we received tensors as A matrices.
                    # Patches mode converted to matrix, or matrix mode used. Need to select accross the spec dimension.
                    # For this node, since it is in matrix mode, the spec dimension is out_c * out_h * out_w
                    ...
                    if alpha_lookup_idx is None:
                        ...
                        # Reshape the spec dimension to c*h*w so we can select used alphas based on unstable index.
                        # Shape becomes [2, out_c, out_h, out_w, batch, *this_layer_shape]
                        selected_lam = selected_lam.view(selected_lam.size(0), *start_node.output_shape[1:], *selected_lam.shape[2:])
                        selected_lam = selected_lam[:, unstable_idx[0], unstable_idx[1], unstable_idx[2]]
                    else:
                        assert alpha_lookup_idx.ndim == 3
                        # We only stored some alphas, and A is also sparse, so the unstable_idx must be first translated to real indices.
                        # alpha shape is (2, sparse_spec_shape, batch_size, *relu_node_shape) where relu_node_shape can also be sparse.
                        # We use sparse-spec alphas. Need to convert these unstable_idx[0], unstable_idx[1], unstable_idx[0] using lookup table.
                        _unstable_idx = alpha_lookup_idx[unstable_idx[0], unstable_idx[1], unstable_idx[2]]
                        selected_lam = self.non_deter_index_select(selected_lam, index=_unstable_idx, dim=1)
                else:
                    # Patches mode. lam must be selected after unfolding, so cannot be done here.
                    # Selection is deferred to maybe_unfold() using alpha_lookup_idx.
                    # For partially shared alpha, its shape is (2, out_c, batch_size, *relu_node_shape).
                    # For full alpha, its shape is (2, out_c*out_h*out_w, batch_size, *relu_node_shape).
                    # Both the spec dimension and relu_node_shape dimensions can be sparse.
                ...
                assert alpha_lookup_idx is None or alpha_lookup_idx.ndim == 1
                _unstable_idx = alpha_lookup_idx[unstable_idx] if alpha_lookup_idx is not None else unstable_idx
                selected_lam = self.non_deter_index_select(self.lam[start_node.name], index=_unstable_idx, dim=1)
            elif unstable_idx.ndim == 2:
                assert alpha_lookup_idx is None, "sparse spec alpha has not been implemented yet."
                # Each element in the batch selects different neurons.
                selected_lam = batched_index_select(self.lam[start_node.name], index=unstable_idx, dim=1)
            else:
                raise ValueError
        else:
            # Spec dimension is dense. Alpha must not be created sparsely.
    ...
    def sdp_crown_bias(self, w, g, lam, start_node, output_shape, sign=+1):
        ...
        if isinstance(w, Patches):
            ...
            # Before unfolding, xc has shape: (batch, in_c, H, W), 
            # After unfolding, xc has shape: (batch, out_h, out_w, in_c, in_h, in_w).
            patch_size = self.patch_size[start_node.name]
            xc = inplace_unfold(xc, (patch_size[-2], patch_size[-1]), w.stride, w.padding, w.inserted_zeros, w.output_padding)
            if w.unstable_idx is not None:
                # w and g have shape: (unstable_out, batch, in_c, in_h, in_w).
                # lam has shape: (unstable_out, batch).
                # append three dimension to the end of lam to match the shape of w and g.
                xc = xc[:, w.unstable_idx[1],w.unstable_idx[2]].transpose(0, 1)
                lam = lam[w.unstable_idx[0],:,w.unstable_idx[1],w.unstable_idx[2], None, None, None]
            else:
                # w and g have shape: (out_c, batch, out_h, out_w, in_c, in_h, in_w).
                # lam has shape: (out_c, batch, out_h, out_w).
                # append three dimension to the end of lam to match the shape of w and g.
                lam = lam[:, :, :, :, None, None, None]
            w = w.patches
            g = g.patches
        else: 
            # This layer is BoundLinear
            if w.ndim == 5:
                # Previous layer is BoundConv
                # w and g have shape (out, batch, in_c, H, W), xc has shape (batch, in_c, H, W) and lam has shape (out, batch)
                # append three dimension to the end of lam to match the shape of w and g.
                lam = lam[:, :, None, None, None]
            else:
                # Previous layer is also BoundLinear
                # w and g have shape (out, batch, in), xc has shape (batch, in) and lam has shape (out, batch)
                # append one dimension to the end of lam to match the shape of w and g.
                lam = lam[:, :, None]

        # Implement Theorem 4.1 in the paper
        psi = torch.minimum(sign*(g-w) - lam*xc, sign*(-g) + lam*xc).clamp_(max=0)
        psi = -lam*xc**2 + psi**2 / lam
        psi = torch.sum(psi, dim=tuple(range(len(output_shape), psi.ndim))).view(output_shape)
        h_sdp = sign*1/2*(lam.view(output_shape)*rho**2 + psi)
        return h_sdp
    ...
    def bound_backward(self, last_lA, last_uA, x=None, start_node=None,
        ...
        # Update the bias using SDP-CROWN if the L2 norm ball is available.
        if self.input_rho is not None and self.opt_stage in ['opt', 'reuse'] and self.L2_stage in ['opt']:
            ...
            if last_uA is not None:
                ubias = self.sdp_crown_bias(last_uA, uA, selected_lam[1], start_node, ubias.shape, sign=+1)
            if last_lA is not None:
                lbias = self.sdp_crown_bias(last_lA, lA, selected_lam[0], start_node, lbias.shape, sign=-1)
