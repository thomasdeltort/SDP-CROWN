...
def _set_lam(optimizable_activations, parameters, lams, out_lr):
    """Set best_variables, variables and parameters list."""
    for node in optimizable_activations:
        lams.extend(list(node.lam.values()))
        node.SDP_CROWN_start()
    # Variables has shape (2, output_shape, batch_dim, node_shape)
    parameters.append({'params': lams, 'lr': out_lr, 'batch_dim': 2})
    # best_variables is a dictionary of dictionary. Each key is the variable
    # for one activation layer, and each value is a dictionary contains all
    # activation layers after that layer as keys.
    best_lams = OrderedDict()
    for m in optimizable_activations:
        best_lams[m.name] = {}
        for lam_m in m.lam:
            best_lams[m.name][lam_m] = m.lam[lam_m].detach().clone()
            m.lam[lam_m].requires_grad_()
    return best_lams
...
def _update_optimizable_activations(
        ...
        reference_idx, idx, alpha, best_alphas, lam, best_lams, deterministic):
    """
    ...
    for node in optimizable_activations:
        ...
        if lam:
            for lam_m in node.lam:
                best_lams[node.name][lam_m] = node.lam[lam_m]
...
def _get_optimized_bounds(
    ...
    # For SDP-CROWN, we use IBP to get L2 norm perturbation for intermediate layers.
    # Currently, L2 norm propagation is only implemented in interval_propagate for
    # BoundLinear and BoundConv, and we assume all the other layers in the network has one-Lipschitz
    # TODO: implement L2 norm propagation in interval_propagate for all the Bound class in operators
    if lam:
        for node in self.nodes():
            # The input node doesn't involve in the propagation.
            if isinstance(node, BoundInput):
                continue
            # Only calculate L2 norm propagation for the perturbed nodes.
            if node.perturbed and hasattr(node, 'output_rho'):
                # For BoundLinear and BoundConv, we propagate L2 norm inside interval_propagate
                ...
                # We assume all the other nodes are has one-Lipschitz
    ...
    if lam:
        best_lams = _set_lam(
            optimizable_activations, parameters, lams, opts['lr_lambda'])
    else:
        best_lams = None
    if beta:
