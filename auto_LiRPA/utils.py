...
def power_iteration(weight, input_shape, output_shape, stride, padding, dilation, groups, num_iterations=100):
    ...
    with torch.no_grad():
        ...
        for _ in range(num_iterations):
            x_k1 = nn.functional.conv2d(x_k, weight, bias=None, stride=stride, padding=padding, dilation=dilation, groups=groups)
            conv_norm = x_k1.reshape(x_k1.size(0), -1).norm(dim=1, keepdim=True).view(-1, 1, 1, 1)
            x_k1 = x_k1 / conv_norm
            x_k = nn.functional.conv_transpose2d(x_k1, weight, bias=None, stride=stride,
